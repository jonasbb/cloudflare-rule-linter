...
pub(crate) fn build_scheme(phase: Phase) -> Scheme {
    ...
    if matches!(
        phase,
        Phase::HttpRequestApiGatewayEarly | Phase::HttpRequestApiGatewayLate | Phase::Maximum
    ) {
        builder
            .add_function(
                "is_jwt_present",
    ...
    if !matches!(
        phase,
        Phase::HttpRequestApiGatewayEarly | Phase::HttpRequestApiGatewayLate
    ) {
        ...
        if matches!(
            phase,
            Phase::HttpRequestLateTransform | Phase::HttpResponseHeadersTransform | Phase::Maximum
        ) {
            builder
                .add_function(
                    "encode_base64",
        ...
        builder
            ...
            .unwrap();
        if matches!(phase, Phase::HttpRequestTransform | Phase::Maximum) {
            builder
                .add_function(
                    "remove_query_args",
        ...
        if matches!(
            phase,
            Phase::HttpCustomErrors | Phase::HttpResponseHeadersTransform | Phase::Maximum
        ) {
            builder
    ...
    match phase {
        ...
        | Phase::HttpRequestSanitize
        | Phase::HttpRequestTransform
        | Phase::HttpRequestApiGatewayEarly
        | Phase::HttpConfigSettings
        ...
        Phase::HttpRequestFirewallCustom => {
            ...
            phase_custom_rules(&mut builder, false);
        ...
        Phase::HttpCustomErrors
        | Phase::HttpResponseHeadersTransform
        | Phase::HttpResponseCompression
        | Phase::HttpResponseFirewallManaged
        | Phase::HttpLogCustomFields => {
            requests_early(&mut builder, false);
            requests_late(&mut builder, false);
...
/// Request metadata that is available since the beginning, e.g., hostname
fn requests_early(builder: &mut wirefilter::SchemeBuilder, #[allow(unused)] is_response: bool) {
...
/// Request metadata specific to the security products and not available afterwards
fn requests_mid(builder: &mut wirefilter::SchemeBuilder, #[allow(unused)] is_response: bool) {
...
/// Some extended metadata available after the security products, e.g., bot information
fn requests_late(builder: &mut wirefilter::SchemeBuilder, #[allow(unused)] is_response: bool) {
...
/// Response metadata only available in the response phases
fn response(builder: &mut wirefilter::SchemeBuilder, #[allow(unused)] is_response: bool) {
...
/// Request metadata that is only available in the custom rules phase
fn phase_custom_rules(builder: &mut wirefilter::SchemeBuilder, #[allow(unused)] is_response: bool) {
